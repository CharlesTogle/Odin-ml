> **Deprecated in this repository.** This is the frontend/TypeScript repository standard for the `odin` application. It does not apply to Odin-ML, which is a Python/FastAPI service. The current Python/ML standard is `../REPOSITORY-STANDARDS.md`. Retained for historical reference and for use in the `odin` application repository.
